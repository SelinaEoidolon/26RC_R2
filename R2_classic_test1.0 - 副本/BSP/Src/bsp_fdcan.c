...
//#define CAN_3508_M7_ID 0x207
//// 定义FDCAN1的目标ID范围（示例）
//#define FDCAN1_TARGET_ID_START 0x100
//#define FDCAN1_TARGET_ID_END   0x1FF
...
//// 激活FDCAN1的中断（主要关注FIFO0）
//HAL_FDCAN_ActivateNotification(&hfdcan1,
//													 FDCAN_IT_RX_FIFO0_WATERMARK | FDCAN_IT_TX_COMPLETE | 
//													 FDCAN_IT_TX_FIFO_EMPTY | FDCAN_IT_BUS_OFF |
//													 FDCAN_IT_ARB_PROTOCOL_ERROR | FDCAN_IT_DATA_PROTOCOL_ERROR |
//													 FDCAN_IT_ERROR_PASSIVE | FDCAN_IT_ERROR_WARNING,
//													 0x00000F00);

//// 激活FDCAN2的中断（主要关注FIFO1）
//HAL_FDCAN_ActivateNotification(&hfdcan2,
//													 FDCAN_IT_RX_FIFO1_NEW_MESSAGE | FDCAN_IT_TX_COMPLETE | 
//													 FDCAN_IT_TX_FIFO_EMPTY | FDCAN_IT_BUS_OFF |
//													 FDCAN_IT_ARB_PROTOCOL_ERROR | FDCAN_IT_DATA_PROTOCOL_ERROR |
//													 FDCAN_IT_ERROR_PASSIVE | FDCAN_IT_ERROR_WARNING,
//													 0x00000F00);
...
//* @param:       void
//* @retval:     	void
//* @details:    	CAN滤波器初始化（FDCAN1->FIFO0，FDCAN2->FIFO1）
//************************************************************************
//**/
//void can_filter_init(void)
//{
...
///************************* FDCAN1 滤波器配置（映射到FIFO0）*************************/
//fdcan_filter.IdType = FDCAN_STANDARD_ID;                       // 标准ID
//fdcan_filter.FilterIndex = 0;                                  // 滤波器索引0
//fdcan_filter.FilterType = FDCAN_FILTER_MASK;                   // 掩码模式
//fdcan_filter.FilterConfig = FDCAN_FILTER_TO_RXFIFO0;           // FDCAN1数据到FIFO0
//fdcan_filter.FilterID1 = FDCAN1_TARGET_ID_START;               // 目标ID起始
//fdcan_filter.FilterID2 = 0xFF0;                                // 掩码（接收0x100-0x1FF）
//HAL_FDCAN_ConfigFilter(&hfdcan1, &fdcan_filter); 		 		 

//// FDCAN1全局过滤器配置：拒绝不匹配的帧
//HAL_FDCAN_ConfigGlobalFilter(&hfdcan1, FDCAN_REJECT, FDCAN_REJECT, 
//												 FDCAN_REJECT_REMOTE, FDCAN_REJECT_REMOTE);
//// 配置FDCAN1 FIFO0水位（1帧即触发中断）
//HAL_FDCAN_ConfigFifoWatermark(&hfdcan1, FDCAN_CFG_RX_FIFO0, 1);

///************************* FDCAN2 滤波器配置（映射到FIFO1）*************************/
//fdcan_filter.FilterIndex = 14;                                 // 滤波器索引14
//fdcan_filter.FilterConfig = FDCAN_FILTER_TO_RXFIFO1;           // FDCAN2数据到FIFO1
//fdcan_filter.FilterID1 = 0x200;                                // 电机ID起始
//fdcan_filter.FilterID2 = 0x000;                                // 掩码0：接收所有0x200开头的ID
//HAL_FDCAN_ConfigFilter(&hfdcan2, &fdcan_filter); 		 		 

//// FDCAN2全局过滤器配置：拒绝不匹配的帧
//HAL_FDCAN_ConfigGlobalFilter(&hfdcan2, FDCAN_REJECT, FDCAN_REJECT, 
//												 FDCAN_REJECT_REMOTE, FDCAN_REJECT_REMOTE);
//// 配置FDCAN2 FIFO1水位（1帧即触发中断）
...
///**
//************************************************************************
//* @brief:      	fdcanx_receive(FDCAN_HandleTypeDef *hfdcan, uint8_t fifo_num, uint16_t *rec_id, uint8_t *buf)
//* @param:       hfdcan：FDCAN句柄
//* @param:       fifo_num：FIFO编号（0或1）
//* @param:       rec_id：接收ID缓存
//* @param:       buf：接收数据缓存
//* @retval:     	接收的数据长度
//* @details:    	从指定FIFO接收数据
//************************************************************************
//**/
//uint8_t fdcanx_receive(FDCAN_HandleTypeDef *hfdcan, uint8_t fifo_num, uint16_t *rec_id, uint8_t *buf)
//{	
//FDCAN_RxHeaderTypeDef pRxHeader;
//uint8_t len = 0;
//uint32_t fifo_id = (fifo_num == 0) ? FDCAN_RX_FIFO0 : FDCAN_RX_FIFO1;

//if(HAL_FDCAN_GetRxMessage(hfdcan, fifo_id, &pRxHeader, buf)==HAL_OK)
//{
//*rec_id = pRxHeader.Identifier;
//if(pRxHeader.DataLength<=FDCAN_DLC_BYTES_8)
//		len = pRxHeader.DataLength;
...
//if (HAL_FDCAN_GetRxMessage(hfdcan, FDCAN_RX_FIFO0, &rx_header, rx_data) == HAL_OK)
//{
//		fdcan1_rx_callback(); // 处理FDCAN1的数据
//}
//}
//}
...
//// FDCAN2的数据会进入FIFO1
//if (hfdcan == &hfdcan2 && (RxFifo1ITs & FDCAN_IT_RX_FIFO1_NEW_MESSAGE) != RESET)
//{
//if (HAL_FDCAN_GetRxMessage(hfdcan, FDCAN_RX_FIFO1, &rx_header, rx_data) == HAL_OK)
//{
//		// 处理电机数据 ID（0x201 ~ 0x207）
//		if (rx_header.Identifier >= CAN_3508_M1_ID && rx_header.Identifier <= CAN_3508_M7_ID)
//		{
//				uint8_t i = (uint8_t)(rx_header.Identifier - CAN_3508_M1_ID);
//				motor_measure_t *m = &motor_fdcan2[i];
