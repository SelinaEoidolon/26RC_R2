...
//    }
    ...
    (void)CDC_App_Write(data, len);
    // 不需要在这里狂调用 TxTask，主循环会一直调
}
...
void Receive(uint8_t bytedata)
{
        ...
        case 3: // 接收命令字节状态
                ...
                usb_Buf[cnt++] = bytedata;
                cmd = bytedata;
                data_ptr = &usb_Buf[cnt]; // 记录数据指针首地址
                if (len == 0)
                        step++; // 数据字节长度为0则跳过数据接收状态
                break;
        case 4: // 接收len字节数据状态
                usb_Buf[cnt++] = bytedata;
                if (data_ptr + len == &usb_Buf[cnt]) // 利用指针地址偏移判断是否接收完len位数据
                {
                        step++;
                }
                break;
        ...
        case 6: // 接收crc16校验低8位字节
                crc16 <<= 8;
                crc16 += bytedata;
                if (crc16 == CRC16_Check(usb_Buf, cnt)) // 校验正确进入下一状态
                {
                        step++;
                }
                else if (bytedata == 0xA5)
