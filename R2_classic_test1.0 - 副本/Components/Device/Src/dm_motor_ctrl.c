...
void dm_motor_init(void)
{
 ...
 //参数设置
	for (int i = Motor1; i <= Motor7; i++)
{
    motor[i].id = 0x01 + i;        // 自动生成ID：Motor1=0x01，Motor2=0x02
    motor[i].mst_id = 0x00 + i;        // 仅标识用
    motor[i].tmp.read_flag = 1;
    
    // 控制参数设置
    motor[i].ctrl.mode     = psi_mode;
    motor[i].ctrl.vel_set  = 1.0f;
    motor[i].ctrl.pos_set  = 6.28f;
...
void fdcan1_rx_callback(void)
{
	...
	switch (rec_id)
	{
		...
		case 0x04: dm_motor_fbdata(&motor[Motor5], rx_data); receive_motor_data(&motor[Motor5], rx_data); break;
		case 0x05: dm_motor_fbdata(&motor[Motor6], rx_data); receive_motor_data(&motor[Motor6], rx_data); break;
		case 0x06: dm_motor_fbdata(&motor[Motor7], rx_data); receive_motor_data(&motor[Motor7], rx_data); break;
		default:break;
	}
}
